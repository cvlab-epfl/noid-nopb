...
class DictMeter(object):
    ...
    def __getitem__(self, key):
        if key not in self.dict_meter:
            log.debug(f"Trying to query a key which doesn't exist in the Meter dictionary, returning default 0 value: {key}")
...
def batch_logging(epoch, batch_index, nb_batch, stats_meter, loss_to_print=[], metric_to_print=[], validation=False):
    ...
    model_time = stats_meter["model_time"].avg
    ...
    time_percent_model = int((model_time / batch_time)*100.0)
    ...
    str_time_lgd = "Time Batch(Data,Model,Crit,Optim)"
    str_time = f"Time  {batch_time:.2f}({time_percent_data:02d}%,{time_percent_model:02d}%,{time_percent_criterion:02d}%,{time_percent_optim:02d}%)\tEnd {time_til_end}"
